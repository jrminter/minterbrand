...
#' @param band_color The color of the SI Logo band.  Defaults to \code{minter_design$granite}.
#' @return A png of the last plot with optional SI logo band.
#' @examples
#' minter_colorplot() + ggplot2::ggtitle("My Title")
#' minter_ggsave(add_logo = TRUE)
#' @export
minter_ggsave <- function(filename = "auto",
                          dir = "auto",
                          plot = ggplot2::last_plot(),
                          width = 6, height = 4, dpi = 300,
                          add_logo = FALSE, logo_height_ratio = .05,
                          band_color = minter_design$granite) {
  ...
  if(add_logo) {
    ...
    fi <- system.file("extdata", "logo_background.png", package = "minterbrand")
    logo <- magick::image_read(fi) %>%
...
#' Show Minter Theme Colors
#' @description Shows the Minter theme colors for reference.
#' @importFrom magrittr "%>%"
#' @return A plot of SI colors
#' @examples
#' minter_colorplot()
#' @export
minter_colorplot <- function() {
  data.frame("color" = names(unlist(minter_design)),
             "code" = unlist(minter_design), stringsAsFactors = F) %>%
    ggplot2::ggplot() +
...
#' @examples
#' minter_ggplot_update()
#' @export
minter_ggplot_theme_update <- function() {
  ggplot2::update_geom_defaults("bar", list(fill = minter_design$dkblue))
  ggplot2::update_geom_defaults("smooth", list(colour = minter_design$dkblue,
                                               fill = minter_design$ltblue,
                                               alpha = I(2/10)))
  ggplot2::update_geom_defaults("point",
                                list(colour = minter_design$dkblue,
                                     fill = minter_design$dkblue))
  ggplot2::update_geom_defaults("col", list(fill = minter_design$dkblue))
